...
import numpy as np
from maha_dist_util.adaptation_network import *
...
class SupContrastReplay(ContinualLearner):
    ...
    def _build_class_reps_and_covariance_estimates(self, context_features, context_labels):
        ...
        for c in torch.unique(context_labels):
            ...
            self.class_precision_matrices[c.item()] = torch.inverse((lambda_k_tau * self.estimate_cov(class_features)) + ((1 - lambda_k_tau) * task_covariance_estimate) + torch.eye(class_features.size(1), class_features.size(1)).cuda(0))
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                for j in range(self.mem_iters):
                    ...
                    if mem_x.size(0) > 0:
                        ...
                        class_means = torch.stack([self.class_representations[label] for label in mem_y.cpu().detach().numpy().tolist()]).squeeze(1)
                        class_precision_matrices = torch.stack([self.class_precision_matrices[label] for label in mem_y.cpu().detach().numpy().tolist()])
                        ...
                        target = split_first_dim_linear(sample_logits, [self.NUM_SAMPLES, batch_x.shape[0]])
                        accuracy = aggregate_accuracy(target, batch_y)
                        #print("accuracy :",accuracy.item())
                        acc_batch.update(accuracy,batch_y.size(0))
                        #acc.extend(accuracy)
                        ###acc.extend(accuracy.item())
                        #################################################
                ...
                if i % 100 == 1 and self.verbose:
                        print(
                            '==>>> it: {}, avg. loss: {:.6f}, avg. acc_batch: {:.6f}'
