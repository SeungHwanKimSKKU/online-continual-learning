...
def compute_logits_radii(cluster_centers, data, radii, prior_weight=1.):
    ...
    Returns:
        log_prob: [B, N, K] logits.
    """
    #data.shape [128, 3, 32, 32]
    print("0_0. cluster_centers.shape,data.shape,radii.shape",cluster_centers.shape,data.shape,radii.shape)
    print("0. data.shape ,cluster_centers.shape:",data.shape,cluster_centers.shape)
    cluster_centers = torch.unsqueeze(cluster_centers, 2)   # [B, 1, K, D]
    print("1. cluster_centers.shape:",cluster_centers.shape)# [B, 1, K]
    data = torch.unsqueeze(data, 2)  # [B, N, 1]
    print("2. data.shape:",data.shape)
    dim = data.size()[-1]
    print("3_0. raddi.shape:",radii.shape)
    radii = torch.unsqueeze(radii, 1)  # [B, 1, K]
    print("3. radii.shape:",radii.shape)# [B, 1, K]
    print("3_1 data.shape,cluster_centers.shape",data.shape,cluster_centers.shape)
    neg_dist = -torch.sum((data - cluster_centers)**2, dim=1)   # [B, N, K]
    print("4. neg_dist.shape:",neg_dist.shape)# [B, N, K]
    print("4_1. radii.shape",radii.shape)
